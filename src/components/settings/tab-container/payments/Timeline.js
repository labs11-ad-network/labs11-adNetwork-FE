        ...
        <>
          {payouts.map(p => {
            return (
              ...
              >
                {`You recieved a payment for the amount of $${p.amount /
                  100} ${moment.unix(p.arrival_date).fromNow()}`}
              </TimelineEvent>
            );
          })}
        </>
          ...
          {payments.map(p => {
            return (
              <TimelineEvent
                ...
                subtitle={`Amount: ${p.amount / 100} Last 4 Digits: ${
              ...
              >
                {`${p.description} of $${p.amount / 100} ${moment
                  .unix(p.created)
                  .fromNow()}. You can find a copy `}
                <a
                  href={p.receipt_url}
                  target="_blank"
